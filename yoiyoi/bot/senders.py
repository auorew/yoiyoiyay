...
from ..bot import READ_MEDIA_TIMEOUT, WRITE_MEDIA_TIMEOUT, pyro_app
...
async def pyro_get_message(update: Update) -> PyroMessage:
...
@retry_request
async def reply_media_group(post: Message, **kwargs) -> Message:
    ...
    return await post.reply_media_group(**kwargs)


@retry_request
async def pyro_send_media_group(message: PyroMessage, **kwargs) -> PyroMessage:
